...
3. **Sieve-parsing verwacht een specifiek template**: precies één `if address :is/:matches "from" [ ... ]`-blok met een `expire "day" "N"`-actie. Regexes staan in `content.js` (`LIST_RE`, `DAYS_RE`). Complexere sieves worden bewust overgeslagen. Bij het wegschrijven wordt `:is` altijd omgezet naar `:matches`, zodat domeinpatronen (`*@site.nl`) werken; voor exacte adressen is het gedrag gelijk.
