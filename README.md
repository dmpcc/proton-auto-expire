...
2. Click **+ new archive rule**. The extension asks after how many days to move the mail, then shows an inline list of destination folders: the system **Archive** folder first, followed by your own custom folders. Nested folders are shown indented under their parent, long lists scroll inside the picker, and a rule displays the full folder path (e.g. "Newsletters/Old") so identically named subfolders stay distinguishable. Pick one.
